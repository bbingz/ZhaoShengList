        ...
        for col_num, _ in enumerate(cells):
            ...
            if col_num == 1:
                ...
                if img:
                    ...
                    # 使用 SwiftOCR 替换 Pytesseract
                    with tempfile.NamedTemporaryFile(suffix=".png") as temp_file:
                        img_processed.save(temp_file.name, "PNG")  # 将预处理后的图像保存为临时 PNG 文件
                        swiftocr_process = subprocess.Popen(["SwiftOCR", temp_file.name], stdout=subprocess.PIPE, stderr=subprocess.PIPE)  # 调用 SwiftOCR 可执行文件，并传递临时图像文件作为参数
                        swiftocr_output, _ = swiftocr_process.communicate()  # 获取 SwiftOCR 的输出和错误信息（如果有）
                        text = swiftocr_output.decode("utf-8").strip()  # 将从 SwiftOCR 获得的输出转换为字符串，并去掉前后空白字符
